...
def average_similarity_ratio(target_string: str, keywords: List[str]) -> float:# Function to calculate the average similarity ratio between a target string and a list of keywords.
    ratio = 0  # Initialize the ratio variable to accumulate similarity scores.

    # Iterating through each keyword in the list.
    for keyword in keywords:
        ratio += fuzz.token_sort_ratio(target_string, keyword) # Increasing the ratio by the similarity score of the target string and the current keyword.

    return ratio / len(keywords) # Returning the average ratio by dividing the accumulated ratio by the number of keywords.

def max_similarity_ratio(target_string: str, keywords: List[str]) -> float: # Function to find the maximum similarity ratio between a target string and a list of keywords.
    ratio = 0  # Initialize the ratio variable to track the highest similarity score.

    # Iterating through each keyword in the list.
    for keyword in keywords:
        ratio = max(fuzz.token_sort_ratio(target_string, keyword), ratio) # Updating the ratio with the maximum value between the current ratio and the new similarity score.
