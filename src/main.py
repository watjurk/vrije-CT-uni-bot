...
def sports_specific_path(user_response_to_first_question: str):  # Path for users who have a specific sport in mind.
    def print_sport_available_message():
        print(bot_format("Great this specific sport is available at the university."))                #if the user's mentioned sport is available in the defined list
        print(bot_format("You can sign-up on the University Sports Centre website: sportcentrumvu.nl"))   #suggests user to go to the university website
...
def activities_path(user_response_to_first_question: str):      # Path for users interested in activities.
    ...
    if not compare.contains_negation_word(user_response_to_first_question):   #checks if the user's response does not have any negative word
        ...
        if compare.contains_exact(user_response_to_first_question, upcoming_keywords):  #if there is an exact match of the keywrod defined then it returns as true
    ...
    if (wants_to_join_association and interested_in_upcoming) or (not wants_to_join_association and not interested_in_upcoming):
        print(bot_format("Are you interested in upcoming events or do you want to join an association?"))    #asks user about interest in upcoming events and association joining
        user_choice = inquiry.checkbox(["Upcoming events", "Joining an association"])                        #defined keywords for the uqestion about upcoming events/association joining
